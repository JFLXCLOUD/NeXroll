...
# Create version.py dynamically using APP_VERSION build arg
ARG APP_VERSION
RUN echo "__version__ = '${APP_VERSION}'" > /app/NeXroll/version.py && \
    echo "def get_version():" >> /app/NeXroll/version.py && \
    echo "    return __version__" >> /app/NeXroll/version.py
