...
class PlexConnector:
    ...
    def _find_config_file(self):
        """Find plex_config.json in current directory or parent directory"""
        config_name = "plex_config.json"

        # First try current directory
        current_dir = os.getcwd()
        current_path = os.path.join(current_dir, config_name)
        if os.path.exists(current_path):
            return current_path

        # Then try parent directory
        parent_dir = os.path.dirname(current_dir)
        parent_path = os.path.join(parent_dir, config_name)
    ...
    def save_stable_token(self, token: str):
        """Save token to plex_config.json"""
        try:
            config_data = {
                "plex_token": token,
    ...
    def test_connection(self) -> bool:
        try:
            ...
            print(f"Using token: {self.token[:10]}...")  # Log first 10 chars for debugging
            response = requests.get(f"{self.url}/", headers=self.headers, timeout=10)
            print(f"Plex response status: {response.status_code}")
            if response.status_code != 200:
                print(f"Plex response content: {response.text[:200]}")
            return response.status_code == 200
        except requests.exceptions.RequestException as e:
            print(f"Plex connection error: {str(e)}")
            return False
        except Exception as e:
            print(f"Unexpected error testing Plex connection: {str(e)}")
            return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                server_info = {
                    "connected": True,
                    "status": "OK",
                    "name": root.get('friendlyName', 'Unknown Server'),
