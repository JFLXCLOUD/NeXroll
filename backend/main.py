...
def upload_preroll(
    ...
    os.makedirs("data/prerolls", exist_ok=True)
    os.makedirs("data/prerolls/thumbnails", exist_ok=True)
    ...
    category_path = f"data/prerolls/{category_dir}"
    thumbnail_category_path = f"data/prerolls/thumbnails/{category_dir}"
    ...
    # Generate thumbnail
    thumbnail_path = f"{thumbnail_category_path}/{file.filename}.jpg"
    try:
        ffmpeg.input(file_path, ss=5).output(thumbnail_path, vframes=1, q=2).run(overwrite_output=True)
        # Store relative path for static serving (remove 'data/' prefix)
        thumbnail_path = thumbnail_path.replace("data/", "", 1) if thumbnail_path else None
    except:
    ...
    try:
        ...
        if preroll.thumbnail:
            ...
            if not os.path.isabs(thumbnail_path):
                thumbnail_path = os.path.join("data", thumbnail_path)
