...
function MatchAudioDuration({ settings, update }) {
  ...
  return (
    <div className="nx-gen-toggle-row">
      ...
      <div>
        ...
        <span>{`Your track runs ${seconds}s. Turning this on sets the duration to match so it plays in full.`}</span>
      </div>
      <StudioSwitch
        checked={matched}
        onChange={checked => update({ duration: checked ? seconds : 10 })}
