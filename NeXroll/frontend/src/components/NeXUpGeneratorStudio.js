...
function MatchAudioDuration({ settings, update, fallbackDuration = 10 }) {
  const seconds = Number(settings.customAudioDuration) || 0;
  if (!settings.customAudioFilename || !seconds) return null;
  const matched = Number(settings.duration) === seconds;
  return (
    <div className="nx-gen-toggle-row">
      <i><Music size={18} /></i>
      <div>
        <strong>Match length to the soundtrack</strong>
        <span>{`Your track runs ${formatSeconds(seconds)}. Turning this on sets the render to that length so it plays in full.`}</span>
      </div>
      <StudioSwitch
        checked={matched}
        onChange={checked => update({ duration: checked ? seconds : fallbackDuration })}
