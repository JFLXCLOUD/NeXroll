        ...
        {connected ? (
          <div className="nx-ap-service-body">
            ...
            <div className="nx-ap-status-line">
              <i>ON</i><div><strong>Automatic trailer downloads</strong><span>Fetch eligible missing trailers during sync.</span></div>
    ...
    <div className="nx-ap-page nx-ap-connections" data-nexup-page="connections">
      ...
      {connectedCount > 0 && (
        <section className="nx-ap-sync-banner">
          <div><h3>{connectedCount === 2 ? 'Radarr and Sonarr are ready' : `${settings.radarr_connected ? 'Radarr' : 'Sonarr'} is ready`}</h3><p>{enabled ? `Automatic syncing is on.${nextSync ? ` Next refresh ${formatTimeAgo(nextSync)}.` : ''}` : 'Automation is paused. Manual sync remains available.'}</p><div className="nx-ap-coverage">{Array.from({ length: 6 }, (_, index) => <i key={index} className={index < (enabled ? 5 : 2) ? 'on' : ''} />)}</div></div>
...
function UpcomingPage(props) {
  ...
  const filtered = useMemo(() => items.filter(item => {
    ...
    return title.includes(search.trim().toLowerCase()) && inWindow && listMatches && (trailerState === 'all' || state === trailerState);
  }), [items, listState, search, source, trailerState, windowDays]);
      ...
      <section className="nx-ap-panel">
        ...
        {view === 'calendar' ? calendarGrid() : (
          <div className="nx-ap-panel-body" data-upcoming-view="list">
            <div className="nx-ap-week-strip">{weekDays.map(day => <div key={dateKey(day)} className={`${dateKey(day) === dateKey(today) ? 'active' : ''}${items.some(item => itemDateKey(item) === dateKey(day)) ? ' has' : ''}`}><span>{day.toLocaleDateString([], { weekday: 'short' })}</span><strong>{day.getDate()}</strong></div>)}</div>
      ...
      <div className={`nx-ap-trailer-layout${previewOn ? ' has-preview' : ''}`}>
        <section className="nx-ap-panel nx-ap-trailer-table">
          {combined.length === 0 ? <EmptyState icon={Film} title="No downloaded trailers" copy="Connect Radarr or Sonarr, then sync eligible trailers." action={settings.radarr_connected || settings.sonarr_connected ? onSync : () => onNavigate('nexup')} actionLabel={settings.radarr_connected || settings.sonarr_connected ? 'Sync trailers' : 'Go to Connections'} /> : <>
            <div className="nx-ap-trailer-head"><span>Trailer</span><span>Duration</span><span>Size</span><span>Usage</span><span>Retention</span><span /></div>
            {filtered.map(trailer => <article key={`${trailer._kind}-${trailer.id}`} className={`${selectedKey === `${trailer._kind}-${trailer.id}` ? 'selected' : ''}${trailer.is_enabled === false ? ' disabled' : ''}`} onClick={() => setSelectedKey(`${trailer._kind}-${trailer.id}`)}><div className="nx-ap-table-media"><button type="button" onClick={event => { event.stopPropagation(); onPlayTrailer({ type: trailer._kind, trailer }); }} aria-label={`Play ${titleForTrailer(trailer)}`}><Play size={12} /></button><div><strong>{titleForTrailer(trailer)}</strong><span>{trailer._kind === 'movie' ? 'Radarr' : 'Sonarr'} / {formatDate(trailer.downloaded_at || trailer.created_at || trailer.release_date)}</span></div></div><span>{formatDuration(trailer)}</span><span>{formatSize(trailer)}</span><Badge tone={trailer.is_enabled !== false ? 'live' : ''}>{trailer.is_enabled !== false ? 'Used' : 'Unused'}</Badge><span>{trailer.removal_date ? formatDate(trailer.removal_date) : 'Keep until release'}</span><div className="nx-ap-row-actions"><button type="button" className="nx-ap-btn" onClick={event => { event.stopPropagation(); onPlayTrailer({ type: trailer._kind, trailer }); }}>Play</button><button type="button" className="nx-ap-btn square" onClick={event => { event.stopPropagation(); toggleSelected(trailer); }} title={trailer.is_enabled !== false ? 'Disable trailer' : 'Enable trailer'}><MoreHorizontal size={13} /></button></div></article>)}
...
function SettingsPage(props) {
  const {
    ...
    onUpdateSettings, onReset, onOpenFolder, onTestPotoken, onConfigureYoutube,
    onInstallPotoken, onTestTmdbKey, tmdbKeyTest, onNotifySaved,
  } = props;
  ...
  return (
    <div className="nx-ap-page nx-ap-settings" data-nexup-page="settings">
      ...
      <div className="nx-ap-save-dock"><span>Changes to individual controls save immediately.</span><div><button type="button" className="nx-ap-btn" onClick={onReset}>Discard</button><button type="button" className="nx-ap-btn primary" onClick={onNotifySaved}>Save NeX-Up settings</button></div></div>
    </div>
  );
}
