...
- **The API wiki page documented an authentication header the server does not accept.** `Authorization: Bearer` returns 401; the server reads `X-Api-Key` or an `api_key` query parameter. The page also listed seven endpoints that no longer exist, including the genre mapping routes removed in beta.6.
