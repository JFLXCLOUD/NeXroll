...
class MediaServerCheckTests(unittest.TestCase):
    def test_no_servers_is_an_error(self):
        check = media_server_check([])
        self.assertEqual(check["status"], ERROR)
        self.assertEqual(score_checks([check]), 70)

    def test_all_configured_servers_must_be_reachable(self):
        check = media_server_check([
            {"name": "Plex", "status": OK, "detail": ""},
            {"name": "Jellyfin", "status": ERROR, "detail": "No connection to media server (Jellyfin)"},
        ])
        summary = build_summary([check])
        self.assertEqual(summary["status"], "degraded")
        self.assertEqual(summary["note"], "No connection to media server (Jellyfin)")
        self.assertEqual(check["value"], "Jellyfin disconnected")

    def test_multiple_healthy_servers_are_supported(self):
        check = media_server_check([{"name": n, "status": OK, "detail": ""} for n in ["Plex", "Jellyfin", "Emby"]])
        self.assertEqual(check["status"], OK)
        self.assertEqual(check["value"], "Plex and Jellyfin and Emby")

    def test_plugin_only_connection_is_unverified(self):
        check = media_server_check([{"name": "Emby", "status": WARN, "detail": "Cannot verify the Emby connection"}])
        self.assertEqual(build_summary([check])["status"], "attention")
        self.assertEqual(check["value"], "Emby unverified")
